...
$(document).ready(function () {
  $("#select-available-resumes").on("change", function () {
    var selectedOption = $(this).val(); // Get the value of the selected option

    // Do something with the selected option value (e.g., display it, perform actions based on it)
    console.log("Selected option:", selectedOption);

    const resumes = JSON.parse(localStorage.getItem(RESUMES_KEY));
    const selectedResume = resumes[selectedOption];
    if (selectedResume)
      document.getElementById("prev").innerHTML = selectedResume;
  });
});
...
//------------- Header -------------------------------------
function makeHeader() {
  document.getElementById("hdr").onclick = function (ev) {
    deletePreviousInfo();
    let form = document.createElement("form");
    form.innerHTML +=
      '<input class="form-control" type="text" name="" id="" placeholder="Name">';
    form.innerHTML +=
      '<input class="form-control" type="text" name="" id="c1" placeholder="Email">';
    form.innerHTML +=
      '<input class="form-control" type="tel" name="" id="c2" placeholder="Mobile">';
    form.innerHTML +=
      '<input class="form-control" type="text" name="" id="" placeholder="Addres">';
    form.innerHTML +=
      '<input class="form-control" type="text" name="" id="c3" placeholder="Linkedin link">';
    form.innerHTML +=
      '<input class="form-control" type="text" name="" id="c4" placeholder="GitHub link">';
    form.innerHTML +=
      '<input class="form-control" type="text" name="" id="c5" placeholder="Portfolio link">';
    form.innerHTML +=
  ...
  document.querySelector("form").onsubmit = function (ev) {
    ...
    for (let i = 0; i < headerInputValues.length; i++) {
      ...
      if (i == 0) {
        ...
        if (i - 1 == 0) {
          headerOutputValues[1].children[i - 1].children[0].setAttribute(
            "href",
            `mailto:${element}`
          );
          headerOutputValues[1].children[i - 1].children[0].textContent =
            element;
        } else if (i - 1 == 3 || i - 1 == 4 || i - 1 == 5) {
          if (element.length > 0) {
            headerOutputValues[1].children[i - 1].children[0].textContent =
              i - 1 == 3 ? "Linkedin" : i - 1 == 4 ? "GitHub" : "Portfolio";
            headerOutputValues[1].children[i - 1].children[0].setAttribute(
              "href",
              element
            );
...
function styleHeader() {
  document.querySelector("#header").style =
    'font-family:"Calibri",sans-serif;margin-bottom:5px;';
...
function makeSummary() {
  document.getElementById("smr").onclick = function (ev) {
    ...
    let form = document.createElement("form");
    form.innerHTML +=
      '<textarea style="word-wrap:break-word;" class="form-control" type="text" name="" id="" placeholder="Suummary" rows="7"></textarea>';
    form.innerHTML +=
      '<input type="submit" id="submit" class="btn btn-info btn-block">';
    document.getElementById("show-form").append(form);
    putSummary();
...
function makeEducation() {
  document.getElementById("edu").onclick = function (ev) {
    deletePreviousInfo();
    ...
    form.innerHTML +=
      '<textarea  oninput="bulletTextArea(event)" rows="7" style="word-wrap:break-word;" class="form-control" placeholder="Description"></textarea>';
    form.innerHTML +=
      '<input class="form-control" type="text" placeholder="Date from-To  ex: Oct 2010 - Jan 2015">';
    form.innerHTML +=
      '<input type="submit" id="submit" class="btn btn-info btn-block">';
    document.getElementById("show-form").append(form);
    putEducation();
...
function putEducation() {
  document.querySelector("form").onsubmit = function (ev) {
    ...
    let eduDiv = document.querySelector(
      "#education .edu-div-all:last-of-type"
    ).children; //2
...
function styleEducation() {
  ...
  document.querySelector(
    "#education"
  ).style = `font-family:"Calibri", sans-serif;margin-bottom:5px;`;
  let divAll = document.querySelectorAll(".edu-div-all");
  for (let i = 0; i < divAll.length; i++) {
    divAll[i].style =
      "display:flex ;flex-wrap:wrap ; justify-content:space-between;margin-bottom:3px;";
  }

  let pre = document.querySelectorAll("#education pre");
  for (let i = 0; i < pre.length; i++) {
    pre[i].style =
      'overflow:hidden;white-space: pre-wrap;margin:0px 0px  3px 10px; font-family:"Calibri",sans-serif; font-size: 1rem;';
  }
...
function makeExperience() {
  document.getElementById("exp").onclick = function (ev) {
    deletePreviousInfo();
    ...
    form.innerHTML +=
      '<textarea  oninput="bulletTextArea(event)" rows="7" style="word-wrap:break-word;" class="form-control" placeholder="Description"></textarea>';
    form.innerHTML +=
      '<input class="form-control" type="text" placeholder="Date from-To  ex: Oct 2010 - present">';
    form.innerHTML +=
      '<input type="submit" id="submit" class="btn btn-info btn-block">';
    document.getElementById("show-form").append(form);
    putExperience();
...
function styleExperience() {
  document.querySelector("#Experience").style =
    'font-family:"Calibri", sans-serif;margin-bottom:5px;';
...
//---------------Skills------------------------------
function makeSkills() {
  document.getElementById("skill").onclick = function (ev) {
    deletePreviousInfo();
    ...
    let form = document.createElement("form");
    form.innerHTML +=
      '<input class="form-control" type="text" placeholder="Categpry ex: Frameworks">';
    form.innerHTML +=
      '<input class="form-control" type="text" placeholder="Skills ex: Spring , React">';
    form.innerHTML +=
      '<input type="submit" id="submit" class="btn btn-info btn-block">';
    document.getElementById("show-form").append(form);
    putSkills();
...
function makeProject() {
  document.getElementById("proj").onclick = function (ev) {
    deletePreviousInfo();
    ...
    form.innerHTML +=
      '<input class="form-control" type="text" placeholder="GitHub Link">';
    form.innerHTML +=
      '<input class="form-control" type="text" placeholder="Live Demo Link">';
    // bug here : input overflow in text area
    form.innerHTML +=
      '<textarea  oninput="bulletTextArea(event)" rows="7" style="word-wrap:break-word;" class="form-control" placeholder="Project description"></textarea>';
    form.innerHTML +=
      '<input type="submit" id="submit" class="btn btn-info btn-block">';
    document.getElementById("show-form").append(form);
    addProject();
...
function addProject() {
  document.querySelector("form").onsubmit = function (ev) {
    ...
    let projectDiv = document.querySelector(
...
function styleProject() {
  ...
  let livelink = document.querySelector(
    "#projects div:last-of-type .live-link"
  );
...
function makeCert() {
  document.getElementById("cert").onclick = function (ev) {
    deletePreviousInfo();
    ...
    form.innerHTML +=
      '<input class="form-control" type="text" placeholder="Oraganization">';
    form.innerHTML +=
      '<textarea  oninput="bulletTextArea(event)" rows="7" style="word-wrap:break-word;" class="form-control" placeholder="Description"></textarea>';
    form.innerHTML +=
      '<input class="form-control" type="text" placeholder="Link">';
    form.innerHTML +=
      '<input class="form-control" type="text" placeholder="Date ex: 2020 or Sep 2022">';
    form.innerHTML +=
      '<input type="submit" id="submit" class="btn btn-info btn-block">';
    document.getElementById("show-form").append(form);
    putCert();
    ...
    for (let i = 0; i < certData.length; i++) {
      if (i == 0 || i == 1 || i == 2) {
        ...
        if (certData[i].value.length > 0) {
          ...
          certLink.textContent = document.querySelector(
            ".cert-div-all:last-of-type .cert-div1 .cert-title"
          ).textContent;
          document.querySelector(
            ".cert-div-all:last-of-type .cert-div1 .cert-title"
          ).textContent = "";
          document
            .querySelector(".cert-div-all:last-of-type .cert-div1 .cert-title")
            .append(certLink);
        }
      } else {
...
function styleCert() {
  document.querySelector("#certificate").style =
    'font-family:"Calibri", sans-serif;margin-bottom:5px;';
...
function makeVolunt() {
  document.getElementById("volunt").onclick = function (ev) {
    deletePreviousInfo();
    let divToAll = document.createElement("div");
...
function putVolunt() {
  document.querySelector("form").onsubmit = function (ev) {
    ...
    let voluntDiv = document.querySelector(
      ".volunt-div-all:last-of-type"
    ).children; //2
    ...
    for (let i = 0; i < voluntData.length; i++) {
      if (i == 0 || i == 1 || i == 2) {
        ...
        if (voluntData[i].value.length > 0) {
          ...
          voluntLink.textContent = document.querySelector(
            ".volunt-div-all:last-of-type .volunt-div1 .volunt-title"
          ).textContent;
          document.querySelector(
            ".volunt-div-all:last-of-type .volunt-div1 .volunt-title"
          ).textContent = "";
...
function styleVolunt() {
  document.querySelector("#volunteering").style =
    'font-family:"Calibri", sans-serif;margin-bottom:5px;';
  ...
  document.getElementById("prev").onclick = function (event) {
    ...
    if (element.tagName != "DIV") {
      ...
      if (element.tagName == "PRE") {
        ...
        form.innerHTML +=
          '<input class="form-control" type="text" placeholder="Edit Here">';
      }
      form.innerHTML +=
        '<input type="submit" id="submit" class="btn btn-info btn-block">';
...
function orderSections() {
  document.getElementById("order-sections").onclick = function (ev) {
    deletePreviousInfo();
    ...
    butDiv.innerHTML =
      ' <button style="height:50px;width:100%" class="btn btn-dark  apply-order" id="ap-ord">Apply new order</button>';
...
function moveSection() {
  ...
  document.querySelector(".old-order").onclick = function (ev) {
...
function applyNewOrder() {
  document.getElementById("ap-ord").onclick = function () {
    let arr = [];
...
document.getElementById("save-with-name").onclick = function save() {
  ...
  const newResumes = { [resumeName]: data, ...resumes };
  localStorage.setItem(RESUMES_KEY, JSON.stringify(newResumes));
  getAvailableResumes();
};
...
function getAvailableResumes() {
  const resumes = JSON.parse(localStorage.getItem(RESUMES_KEY)) || {};
  $("#select-available-resumes").empty();
  // Loop through the options list

  $.each(
    [" Available Resumes", ...Object.keys(resumes)],
    function (index, value) {
      let newOption = $("<option>", {
        value: value,
        text: value,
      });
      $("#select-available-resumes").append(newOption);
    }
  );
}
